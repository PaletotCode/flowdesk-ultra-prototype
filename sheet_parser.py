...
def parse(df_raw: pd.DataFrame, debug: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, List[str]]:
    ...
    # Encontra o índice do primeiro cabeçalho principal para normalizar colunas
    ...
    while i < n:
        ...
        # Um bloco de pedido pode começar com um cabeçalho repetido ou diretamente com os dados
        is_header = "Tipo" in row.values and "Id" in row.values
        
        order_data_row_index = -1
        if is_header:
            order_data_row_index = i + 1
        # Ou se a primeira coluna já é um tipo de pedido válido
        ...
        if order_data_row_index != -1 and order_data_row_index < n:
            ...
            # 1. Pular a linha vazia (flag de início dos itens)
            ...
            # 2. A próxima linha deve ser o cabeçalho dos itens
            if i < n and not _is_blank_row(df.iloc[i]):
                item_header_raw = df.iloc[i, 1:13] # Faixa B:M
                    ...
                    while i < n:
                        ...
                        if _is_blank_row(item_row):
                            ...
                            if blanks >= 2:
                                ...
                                i += 1 # Pula a segunda linha vazia
                                break
                            i += 1
                            continue
                        blanks = 0
                        ...
                        # Tenta obter desconto de duas colunas possíveis
